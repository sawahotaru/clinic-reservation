...
function settingDefaults(): array
{
    return [
        ...
        // ===== リマインダー（来院前のお知らせメール） =====
        // 予約日の N 日前の H 時を過ぎたら、メールを入れているお客さんへ1通だけ送る。
        // 送信済みは reminder_log テーブルで管理するので、何度実行しても二重に送らない。
        'reminder_enabled'     => '0',  // '1'で有効（既定OFF。設置直後に勝手に送らない）
        'reminder_days_before' => '1',  // 何日前に送るか。0=当日
        'reminder_send_hour'   => '18', // その日の何時を過ぎたら送るか（0-23）
